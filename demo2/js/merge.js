...
function errorMessage(str) {
	...
	var html = '<div class="errorMessage MT20" style="bottom: 100px; position: fixed;font-size:16px;text-align: center; width: 100%; display:none; z-index:1111;"><font style="background-color: rgba(0, 0, 0, 0.7); border-radius: 5px; color: #fff; z-index:1000; display: inline-table; line-height: 170%; max-width: 80%; padding: 5px 10px;">' + str + '</font></div>';
	$("body").append(html);
...
//弹框 alertBox("提示框的标题","提示文字","method"); type = 1 有取消按钮; type = 2 没有取消按钮;
function alertBox2(title, tips, method, type) {
	var html = '';
	html += '<div class="width100 height100P positionF" id="alertBox" style="background:rgba(0,0,0,0.3);left:0;top:0;z-index: 1001;">';
	html += '<div class="radius5 whiteBkg width80 positionA textC" style="height:190px;left:10%;top:50%;margin-top: -95px; box-shadow: 0px 0px 3px #666;">';
	html += '<h3 class="font20 PT15 redTex">' + title + '</h3>';
	html += '<p class="font16 blackTex lineHeight1_5x" style="height:75px;padding:15px 5% 0;">' + tips + '</p>';
	if(type == 1) {
		html += '<a class="inlineB grayBkg width35 ML10 MR10 grayTex PT5 PB5 font16" onclick="$(\'#alertBox\').remove();">取消</a>';
	}
...
function alertBox1(tips, method1, tipsMethod1, method2, tipsMethod2, type) {
	var html = '';
	html += '<div class="width100 height100P positionF alertBoxWrap" id="alertBox" style="background:rgba(0,0,0,0.3);left:0;top:0;z-index: 2001;">';
	html += '<div class="radius5 whiteBkg width80 positionA textC" style="height:190px;left:10%;top:50%;margin-top: -95px; box-shadow: 0px 0px 3px #666;">';
	html += '<h3 class="font20 PT15 redTex">提&nbsp;示</h3>';
	html += '<p class="font16 blackTex lineHeight1_5x" style="height:75px;padding:15px 5% 0;">' + tips + '</p>';
	if(type == 1) {
...
//弹框 alertBox("提示文字","method"); type = 1 有两个按钮; type = 2 没有取消按钮;
function alertBox3(tips, method1, tipsMethod1, method2, tipsMethod2, type, methods2Params) {
	var html = '';
	...
	html += '<a class="inlineB redBkg width35 ML10 MR10 whiteTex PT5 PB5 font16" onclick="' + method2 + '(\'' + methods2Params + '\');">' + tipsMethod2 + '</a>';
	html += '</div></div>';
	$("body").append(html);
}
...
//弹窗 alertVerification
/* paramHtml
 * phoneNum 手机号
 * type		登录方式
 */
function alertVerification(paramHtml, phoneNum, type) {

	var tmpVerification = '<div style="margin:20px auto;margin-bottom:0px;" id="captcha_div"></div>';
	paramHtml = paramHtml || tmpVerification;
	var modelHtml = '<div id="Verification" style="width:100%;height:100%;background:rgba(0,0,0,0.5);position:fixed;top:0;z-index:10000;">' +
		'<div style="position:absolute;top:50%;background:white;margin:0 10%">' +
		paramHtml +
		'<div id="verificationStatus" style="width:100%;margin-top:0px;height:30px;line-height:30px;font-size:14px;"></div>' +
		'</div>' +
		'</div>';
	$('body').append(modelHtml);

	var TmpWidth = $('body').width() * 0.8;
	var opts = {
		...
		"verifyCallback": function(ret) { // 用户只要有拖动/点击，就会触发这个回调
			if(ret['value']) { // true:验证通过 false:验证失败
				...
				$("#verificationStatus").html('验证成功');
				$("#verificationStatus").css('color', '#8ec571');
				//最后一次的验证码
				var NECaptchaValidate = ins.getValidate();
				NECaptchaValidate = window.encodeURIComponent(NECaptchaValidate);
				ajaxRequest(contextPath + "/wxuser/goGetCaptcha", "phoneNum=" + phoneNum + "&type=" + type + "&NECaptchaValidate=" + NECaptchaValidate + "&captchaId=32a42600dad34c6f92dc9b3eb6b47f49", goGetCaptcha);
			} else {
				$("#verificationStatus").html('验证失败');
				$("#verificationStatus").css('color', '#fa5b5b');
			}
		}
	};
	ins = new NECaptcha(opts);
	$('.ncpt_puzzle_bg>img').css('border-radius', '8px 8px 0 0');
}

$(document).ready(function() {
	var isUpdatePsdFirst = localStorage.getItem("isUpdatePsdFirst");
	var needUpdatePassword = $("#needUpdatePassword").val();
	var passwordMessage = $("#passwordMessage").val();
	if((needUpdatePassword == "true" || needUpdatePassword == true) && isUpdatePsdFirst) {
		alertBox1(passwordMessage, "dontUpdatePsd", "忽略", "toUpdatePsd", "立即修改", 1);
		localStorage.removeItem("isUpdatePsdFirst");
	}
});

function dontUpdatePsd() {
	$('#alertBox').remove();
}

function toUpdatePsd() {
	window.location.href = contextPath + "wxuser/goAmendLoginPassword";
}

//根据报错等级来操作
function message_level(data, errorMessageFun, alertBox2Fun) {
	if(data.errorMessage) { // 如果后台返回 存在errorMessage
		if(data.errorMessage.message_level == "1") { //data -> 接口返回的data
			errorMessage(data.errorMessage.message_content);
			errorMessageFun(); //errorMessageFun -> 吐司后执行的方法
		} else if(data.errorMessage.message_level == "2") {
			alertBox2("温馨提示", data.errorMessage.message_content, alertBox2Fun, 2); //alertBox2Fun -> alertBox2 里的 '确认' 方法
		}
	} else { // 不存在 errorMessage
...
function toPercent(data) {
	...
	strData = Math.floor(strData);
	var ret = strData.toString() + "%";
	return ret;
}

var phoneReg=/^\d{11}$/;//手机号校验规则 前端只限定11位数字